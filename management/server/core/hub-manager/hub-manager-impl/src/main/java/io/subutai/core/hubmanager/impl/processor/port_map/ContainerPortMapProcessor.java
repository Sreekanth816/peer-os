...
public class ContainerPortMapProcessor implements StateLinkProcessor
{
    ...
    private void processLink( String stateLink )
    {
        ...
        try
        {
            ...
            {
                log.error( "Could not get port map data from Subutai Bazaar" );
            }
            ...
            log.info( !restRes.isSuccess() ? "Could not send port map data to Subutai Bazaar" : "Sent port map data to Subutai Bazaar" );
        }
        catch ( Exception e )
        {
            log.error( "*********", e );
